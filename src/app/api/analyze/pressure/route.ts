...
import { PRESSURE_ANALYSIS_TOKEN_COST } from "@/lib/token-constants";
...
export async function POST(req: Request) {
  ...
  if (!bypassTokens) {
    ...
    if (balance < PRESSURE_ANALYSIS_TOKEN_COST) {
  ...
  const inserted = await db
    ...
    .values({
      ...
      kind: "pressure",
      status: "processing",
      tokensUsed: bypassTokens ? 0 : PRESSURE_ANALYSIS_TOKEN_COST
    })
  ...
  try {
    const pythonRes = await fetch(buildPythonApiUrl(req.url, "/v1/analyze/pressure"), {
      method: "POST",
      body: pythonFormData,
