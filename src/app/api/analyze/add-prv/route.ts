...
export async function POST(req: Request) {
  ...
  const prvRecommendationsRaw = formData.get("prvRecommendations");
  if (typeof prvRecommendationsRaw !== "string") {
    return NextResponse.json({ error: "prvRecommendations tidak valid" }, { status: 422 });
  }

  let prvRecommendations: unknown;
  try {
    prvRecommendations = JSON.parse(prvRecommendationsRaw);
  } catch {
    return NextResponse.json({ error: "prvRecommendations tidak valid" }, { status: 422 });
  }

  if (!Array.isArray(prvRecommendations)) {
    return NextResponse.json({ error: "prvRecommendations tidak valid" }, { status: 422 });
  }

  if (prvRecommendations.length === 0) {
    return NextResponse.json({ error: "Tidak ada rekomendasi PRV" }, { status: 422 });
  }
  ...
  pythonFormData.set("prv_recommendations", JSON.stringify(prvRecommendations));

  try {
    const pythonRes = await fetch(buildPythonApiUrl(req.url, "/v1/analyze/add-prv"), {
      method: "POST",
      body: pythonFormData,
