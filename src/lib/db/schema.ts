...
export const authOtpCodes = pgTable(
  ...
  (table) => ({
    emailPurposeIdx: index("auth_otp_codes_email_purpose_idx").on(
      table.email,
      table.purpose
    )
  })
...
export const contactMessages = pgTable(
  "contact_messages",
  {
    ...
    status: text("status").notNull().default("open"), // open | resolved | spam
...
export const adminTokenEvents = pgTable(
  "admin_token_events",
  {
    id: serial("id").primaryKey(),
    userId: text("user_id")
    ...
    kind: text("kind").notNull(), // grant | refund | revoke | set
