# CANONICAL SOURCE: scripts/epanet/reporter.py — keep in sync
"""
reporter.py â€” Ekspor laporan analisis Markdown per iterasi
"""
...
def _pipe_table(
    ...
    """Tambahkan tabel pipa (before vs after) ke list md. Pipa yang diubah ditandai â˜…."""
    md.append(
    ...
    for pid in sorted(wn.pipe_name_list):
        ...
        mark  = " â˜…" if pid in changed_pipes else ""
        md.append(
...
def export_markdown_report(
    ...
    md += [
        ...
        f"| Status konvergensi | {'**CONVERGED âœ“**' if converged else '**NOT CONVERGED âœ—**'} |",
    ...
    # -----------------------------------------------------------------------
    STATUS_LABEL = {
        "CONVERGED": "SELESAI âœ“ â€” semua kriteria terpenuhi",
        "STUCK":     "BERHENTI â€” tidak ada perubahan yang mungkin",
        "STAGNANT":  "BERHENTI â€” tidak ada kemajuan",
        "RUNNING":   "Berjalan",
    }
    ...
    if optimize_ran:
        for snap in snapshots:
            ...
            md += [
                ...
                f"## Iterasi {snap['iter']}", "",
                f"> Status: **{label}**  ",
                f"> Pelanggaran: **{len(ev_b['violations'])}** â†’ **{len(ev_a['violations'])}**",
                "",
            ]
            ...
            if snap["changes"]:
                ...
                md += ["",
                       "_â˜… Pipa yang diubah ditandai bintang (â˜…) di tabel pipa di bawah._",
                       ""]
            else:
            ...
            if ev_a["violations"]:
                ...
                md += ["**Semua kriteria terpenuhi setelah iterasi ini.** âœ“", ""]
    ...
    if after_eval["violations"]:
        ...
        if prv_candidates:
            ...
            for nid in prv_candidates:
                ...
                md.append(f"- **Node {nid}**: tekanan {p:.2f} m > {PRESSURE_MAX} m â†’ Pasang PRV")
            md.append("")
    else:
        md += ["**Tidak ada pelanggaran tersisa.** "
               "Jaringan memenuhi semua standar Permen PU No. 18/2007. âœ“", ""]

    md += ["---",
           "_Laporan dihasilkan oleh EPANET Network Optimizer â€” Standar: Permen PU No. 18/2007_"]
