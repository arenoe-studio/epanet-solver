"""
diameter.py — Utilitas diameter pipa & Analytical Window Method

Analytical Window Method:
  Untuk setiap pipa, hitung diameter standar optimal secara langsung
  dari debit aktual — tanpa trial-and-error bolak-balik.
  ...
  Tiga kasus:
    FEASIBLE  — ada diameter standar dalam window
    INFEASIBLE — D_max_V < 40 mm (flow terlalu kecil)
    CONFLICT  — D_min_HL > D_max_V (prioritaskan HL, terima V-LOW)
"""
...
# ===========================================================================
# ANALYTICAL WINDOW METHOD — fungsi bantu
# ===========================================================================
...
def hw_d_min_for_hl(
    ...
    """
    Diameter minimum (m) agar HL ≤ hl_max_per_km — Hazen-Williams terbalik.
...
def d_max_for_vmin(flow_m3s: float, v_min: float = VELOCITY_MIN) -> float:
    """
    Diameter maksimum (m) agar V ≥ v_min — persamaan kontinuitas.
...
# ===========================================================================
# ANALYTICAL WINDOW METHOD — fungsi utama
# ===========================================================================
...
def analytical_optimal_diameter(
    ...
    Returns: (diameter_m, kategori)
      "FEASIBLE"   — diameter memenuhi V_min, V_max, dan HL_max
      "INFEASIBLE" — flow terlalu kecil; pakai D_min (40 mm), perlu solusi operasional
      "CONFLICT"   — tidak ada D standar yang memenuhi keduanya; prioritaskan HL
    """
    Q = abs(flow_m3s)
